...
class ProductUpdateViewModelTest {
    ...
    lateinit var viewModel: ProductUpdateViewModel
    ...
    fun setUp() {
        ...
        viewModel = ProductUpdateViewModel(repository, repositoryBrand, repositoryCategory)
    }
    ...
    @Test
    fun `loadProduct should update product state`() = runTest {
        //Given
        val id = 201L
        val product = MotherObjectProduct.productDetailed
        coEvery { repository.getProductById(id) } returns ResultPattern.Success(product)

        turbineScope {
            val turbineProduct = viewModel.product.testIn(backgroundScope)

            // Initial State
            assertTrue(turbineProduct.awaitItem() == null)

            // Call the function to create the category
            viewModel.loadProduct(id)

            // Second State
            assertTrue(turbineProduct.awaitItem() == product)

            turbineProduct.cancel()
        }
        coVerify(exactly = 1) { repository.getProductById(id) }
    }

    @Test
    fun `loadProduct should update message state if error is receive`() = runTest {
        //Given
        val id = 201L
        coEvery { repository.getProductById(id) } returns ResultPattern.Error(null,"error")
        ...
        turbineScope {
            ...
            // Second State
            assertTrue(turbineMessage.awaitItem() == "error")
    ...
    @Test
    fun `updateProduct should update message`() = runTest {
        // Given
        val product = MotherObjectProduct.product
        coEvery { repository.updateProduct(product) } returns "Producto actualizado exitosamente"
        ...
        turbineScope {
            ...
            // Second State
            assertTrue(turbineMessage.awaitItem() == "Producto actualizado exitosamente")
